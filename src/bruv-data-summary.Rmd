...
mean_rich_table_clean <- mean_richness_table %>%  
  subset(select=-geog) %>% 
  kable(digits=2, 
        col.names = c("Site", "Type", "Mean species richness", "SD", "Min", "Max", "Median", "n")) %>% 
  kable_styling()
...
***Figure 5. ** MaxN statistic agregated by family for each site. Sites are ordered MPA first and reference second, with pairs in order of decreasing latitude. Stacked colors indicate unique families.*
...
***Table 3.** Summary statistics of mean maxN of species at each site out of n deployments. This average includes maxN = 0 for deployments in which a species known to be present at the site is absent.*
```{r}
...
***Figure 7 (a-e).** Mean MaxN of all species observed at each site for each site pair. Error bars were left off because of relatively large standard deviation (see Table 3). *
