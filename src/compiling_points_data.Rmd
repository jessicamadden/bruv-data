...
point_data <- point_data %>% 
  mutate(five_min = case_when(
    time >= ms("00:00.00") & time <  ms("05:00.00") ~ 1, 
    time >= ms("05:00.00") & time <  ms("10:00.00") ~ 2, 
    time >= ms("10:00.00") & time <  ms("15:00.00") ~ 3, 
    time >= ms("15:00.00") & time <  ms("20:00.00") ~ 4, 
    time >= ms("20:00.00") & time <  ms("25:00.00") ~ 5, 
    time >= ms("25:00.00") & time <  ms("30:00.00") ~ 6, 
    time >= ms("30:00.00") & time <  ms("35:00.00") ~ 7, 
    time >= ms("35:00.00") & time <  ms("40:00.00") ~ 8, 
    time >= ms("40:00.00") & time <  ms("45:00.00") ~ 9, 
    time >= ms("45:00.00") & time <  ms("50:00.00") ~ 10, 
    time >= ms("50:00.00") & time <  ms("55:00.00") ~ 11, 
    time >= ms("55:00.00") & time <=  ms("60:00.00") ~ 12
  ))
...
point_data <- point_data %>% 
  mutate(ten_min = case_when(
    time >= ms("00:00.00") & time <  ms("10:00.00") ~ 1, 
    time >= ms("10:00.00") & time <  ms("20:00.00") ~ 2, 
    time >= ms("20:00.00") & time <  ms("30:00.00") ~ 3, 
    time >= ms("30:00.00") & time <  ms("40:00.00") ~ 4, 
    time >= ms("40:00.00") & time <  ms("50:00.00") ~ 5, 
    time >= ms("50:00.00") & time <=  ms("60:00.00") ~ 6
  ))
 
```

```{r}
# Case study: Percos barred surfperch and speckled sand dabs
# Get mean maxN for each 5 min interval

perc_mean_maxn_five <- point_data %>% 
  filter(site_full == "Percos") %>%
  filter(common %in% c("Barred surfperch", "Speckled sanddab")) %>% 
  group_by(op_code, five_min, common, sp_code) %>% 
  summarise(
    maxn = max(number)
  ) %>% 
  group_by(five_min, common) %>% 
  summarize(
    mean_maxn = mean(maxn)
  )
...
perc_mean_maxn_ten <- point_data %>% 
  filter(site_full == "Percos") %>%
  filter(common %in% c("Barred surfperch", "Speckled sanddab")) %>% 
  group_by(op_code, ten_min, common, sp_code) %>% 
  summarise(
    maxn = max(number)
  ) %>% 
  group_by(ten_min, common) %>% 
  summarize(
    mean_maxn = mean(maxn)
  )
```
...
```{r}
ggplot(perc_mean_maxn_five, aes(x = five_min, y = mean_maxn, fill = common)) +
  geom_bar(stat="identity", 
           position = "dodge")
```

```{r}
ggplot(perc_mean_maxn_ten, aes(x = ten_min, y = mean_maxn, fill = common)) +
  geom_bar(stat="identity", 
           position = "dodge")
```
...
write.csv(max_n_data, "file:///C:/Users/Crab Lab/Box/Crab Lab Group Folder/MPA Project/mpa_project_code/bruv-data/data//point_data.csv")
```
